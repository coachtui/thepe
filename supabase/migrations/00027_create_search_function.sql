-- Migration: Create Vector Search Function
-- Created: 2026-01-28
-- Purpose: Create PostgreSQL function for semantic document search using pgvector

-- Function: search_documents
-- Performs cosine similarity search on document embeddings
-- Returns chunks ranked by similarity to query embedding

CREATE OR REPLACE FUNCTION search_documents(
  query_embedding vector(1536),
  match_count integer DEFAULT 10,
  similarity_threshold float DEFAULT 0.5,
  filter_project_id uuid DEFAULT NULL
)
RETURNS TABLE (
  chunk_id uuid,
  document_id uuid,
  chunk_index integer,
  content text,
  page_number integer,
  similarity float,
  document_filename text,
  project_id uuid
)
LANGUAGE plpgsql
AS $$
BEGIN
  RETURN QUERY
  SELECT
    dc.id AS chunk_id,
    dc.document_id,
    dc.chunk_index,
    dc.content,
    dc.page_number,
    1 - (de.embedding <=> query_embedding) AS similarity,
    d.filename AS document_filename,
    d.project_id
  FROM document_embeddings de
  JOIN document_chunks dc ON de.chunk_id = dc.id
  JOIN documents d ON dc.document_id = d.id
  WHERE
    (filter_project_id IS NULL OR d.project_id = filter_project_id)
    AND (1 - (de.embedding <=> query_embedding)) >= similarity_threshold
  ORDER BY de.embedding <=> query_embedding ASC
  LIMIT match_count;
END;
$$;

-- Add helpful comment
COMMENT ON FUNCTION search_documents IS
  'Phase 2: Semantic search function using cosine similarity (pgvector <=> operator) on document embeddings';

-- Create index for faster vector similarity search
-- This dramatically improves query performance for large datasets
CREATE INDEX IF NOT EXISTS idx_document_embeddings_vector
  ON document_embeddings
  USING ivfflat (embedding vector_cosine_ops)
  WITH (lists = 100);

-- Add comment for index
COMMENT ON INDEX idx_document_embeddings_vector IS
  'Phase 2: IVFFlat index for fast cosine similarity search on embeddings';
